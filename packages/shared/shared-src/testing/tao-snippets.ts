/** SNIPPET_ACTION_BUMP_STEP_NUMBER is minimal action with local `Step is number` for parser/formatter tests (spacing matches formatter tabSize 3). */
...
/** SNIPPET_ACTION_BUMP_AND_USE_DOT_STEP_FORMATTED is expected output for {@link SNIPPET_ACTION_BUMP_AND_USE_DOT_STEP_RAW} (indents use formatter tabSize 3). */
...
ui V { action Use { do Bump Step 1 } }
`.trim()
...
state Counter = 0
action Bump Step is number { set Counter += Step }
ui V { action Use { do Bump Bump.Step 2 } }
`.trim()
...
state Counter = 0
action Bump Step is number { set Counter += Step }
ui V { action Use { do Bump .Step 3 } }
`.trim()
