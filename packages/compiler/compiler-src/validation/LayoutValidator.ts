...
function viewDirection(node: LayoutHost): TaoLayoutDirection | undefined {
  ...
  const targetName = renderHostTargetView(node)?.name
  return targetName === undefined ? undefined : standardContainerDirection(targetName)
}

function layoutHostViewName(node: LayoutHost): string {
  if (AST.isViewDeclaration(node)) {
    const host = viewDeclarationChildrenHost(node)
    return renderHostTargetView(host)?.name ?? host?.view?.$refText ?? '<unknown>'
  }
  return renderHostTargetView(node)?.name ?? node.view?.$refText ?? '<unknown>'
}
...
function viewDeclarationChildrenHostDirection(node: AST.ViewDeclaration): TaoLayoutDirection | undefined {
  const host = viewDeclarationChildrenHost(node)
  return host === undefined ? undefined : viewDirection(host)
}
...
function renderHostTargetView(node: LayoutRenderHost | undefined): AST.ViewDeclaration | undefined {
  const target = node?.view?.ref
  return isViewLikeDeclaration(target) ? resolveVariantTargetView(target) : undefined
}
