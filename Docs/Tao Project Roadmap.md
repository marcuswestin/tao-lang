...
| 4        | Control Syntax and Statements        | Ready to merge | [Research](./Projects/Control%20Syntax%20and%20Statements%20Project%20Research.md)                                      | [Mini](./Projects/Control%20Syntax%20and%20Statements%20Mini%20Project%20Plan.md); [Full](./Projects/Control%20Syntax%20and%20Statements%20Project%20Plan.md) | Run `project-8-finalize-merge`        | Mini slice implemented: if/else, comparisons, empty states, pure functions                |
...
- Status: Ready to merge for the mini implementation slice; the broader follow-up plan remains planned. Existing implementation includes declarations, aliases, expressions, render statements, view/action blocks, action invocation, bare guard, list iteration, data mutations, navigation actions, validation, comparisons, statement-form `if` / `else`, whole-view reactive-read hoisting for nested render control, and pure top-level functions/returns.
- MVP remaining: settle and implement `when`, guard/check/boundary semantics, event/form statement flow, optional/missing checks, broader call/argument rules, and remaining diagnostics.
