...
| -------- | ------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ------------------------------------------------------- |
| 1        | Data Schema and Queries MVP           | Planned     | -                                                                                                                       | [Plan](./Projects/Data%20Schema%20and%20Queries/Queries%20MVP%20Plan.md)                                        | Run `project-4-review-project-plan`   | Finish provider capability validation and remaining MVP |
| 2        | Navigation and routing                | Selected    | -                                                                                                                       | [WIP](./Projects/Design%20WIP/App%20Routing%20and%20Navigation.md)                                              | Run `project-2-research-project`      | Define and implement Tao navigation correctly           |
...
- Status: Design: navigation has an active WIP doc and is explicitly separate from layout/error handling; Implementation: the Expo runtime already uses Expo Router and includes React Navigation, linking, screens, safe-area, and gesture dependencies, but Tao language-level routing is not yet complete.
- MVP remaining: design route declarations, typed params, deep links, tabs/stacks, route guards, route loading/error behavior, and how app-level navigation composes with view-level state.
- Deferred: complex nested routers, advanced transitions, universal-link policy, multi-window/deep platform routing, and generated route documentation.
- Project docs: [App Routing and Navigation](./Projects/Design%20WIP/App%20Routing%20and%20Navigation.md).
...
- Status: Design: schema/query/write semantics, query identity, provider boundaries, and guard-based loading are actively planned; Implementation: data blocks, raw and structured queries, query aliases, `for`, `guard`, `create`, provider overrides, schema push, Memory, and InstantDB paths have substantial MVP work.
- MVP remaining: finish `if`/`else`, `update`, relationship-heavy target app coverage, provider capability validation, and the literal target app path.
- Deferred: projections, pagination, delete, aggregation, query-on-query, generic provider layer, explicit invalidation DSL, and broad REST/GraphQL/Supabase support.
- Project docs: [Queries MVP Plan](./Projects/Data%20Schema%20and%20Queries/Queries%20MVP%20Plan.md), [Data and Queries - Design](./Projects/Data%20Schema%20and%20Queries/Process%20Docs/Data%20and%20Queries%20-%20Design.md), [Queries MVP Target App](./Projects/Data%20Schema%20and%20Queries/Queries%20MVP%20Target%20App.tao).

## Data Providers, Auth/Session, Local Storage, Offline Behavior, Sync, And Relationships

- Status: Design: provider-owned execution, TanStack/InstantDB mapping, relationship loading, runtime query values, and provider auth boundaries are documented; Implementation: Memory and InstantDB providers exist, runtime admin secrets are stripped from generated app code, and native storage/cache dependencies are present.
