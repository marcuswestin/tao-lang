...
4. Control Syntax and Statements
...
| 4        | Control Syntax and Statements        | Reviewed    | [Research](./Projects/Control%20Syntax%20and%20Statements%20Project%20Research.md)                                      | [Mini](./Projects/Control%20Syntax%20and%20Statements%20Mini%20Project%20Plan.md); [Full](./Projects/Control%20Syntax%20and%20Statements%20Project%20Plan.md) | Run `project-5-implement-project`     | Mini slice first: if/else, comparisons, empty states, pure functions                      |
...
## Control Syntax And Statements

- Status: Reviewed for the mini implementation slice; the broader follow-up plan remains planned. Existing implementation includes declarations, aliases, expressions, render statements, view/action blocks, action invocation, bare guard, list iteration, data mutations, navigation actions, and validation. The reviewed mini slice covers comparisons, `.Empty`, statement-form `if` / `else`, whole-view reactive-read hoisting for nested render control, and pure top-level functions/returns.
- MVP remaining: settle and implement `if` / `else`, `when`, function declarations/returns, guard/check/boundary semantics, event/form statement flow, optional/missing checks, call/argument rules, and diagnostics.
- Deferred: richer pipelines, destructuring, arbitrary loops, macros, full exception systems, transactions, resource scopes, task blocks, broad server-side language features, and non-UI-app language expansion.
- Project docs: [Research](./Projects/Control%20Syntax%20and%20Statements%20Project%20Research.md), [Mini Plan](./Projects/Control%20Syntax%20and%20Statements%20Mini%20Project%20Plan.md), [Full Plan](./Projects/Control%20Syntax%20and%20Statements%20Project%20Plan.md), [Language Spec - Syntax Semantics](./Tao%20Language%20Design/Language%20Spec%20-%20Syntax%20Semantics.md).
...
- Status: Design: semantic, pathless-by-default navigation v1 is specified; Implementation: app `navigation`, top-level `navigator`, `stack`, `tabs`, `screen`, `tab`, primitive `param`, optional `path`, navigation actions, React Navigation IR/codegen/runtime lowering, example migration, headless scenario support, and direct non-Expo-Router Expo entry are implemented on the current branch.
- MVP remaining: complete implementation review, merge preparation, and merge of the navigation v1 branch.
