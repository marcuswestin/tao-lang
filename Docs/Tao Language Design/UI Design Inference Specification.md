...
Optional LLM-backed generation is also CLI-only:

- `tao design --llm <path>` asks the configured LLM provider to generate richer theme suggestions;
- `tao design --llm-provider codex-cli|claude-cli <path>` selects the local provider;
- `--model <model>` overrides the provider default;
- LLM providers must return the same lock-entry shape as deterministic suggestions after validation;
- provider failures must not overwrite existing lock files;
- automated tests use fixture providers and mocked command runners, never network calls or local model CLIs.
