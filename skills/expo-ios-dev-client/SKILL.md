...
- Before running `expo run:ios --device`, the launcher should detect whether a matching physical iPhone is connected through `xcrun devicectl list devices --json-output`; if not, skip physical iPhone install/open and keep the dev server running.
