---
name: check-all-changes-for-improvements
...
```
./agent git status --short
./agent git diff
./agent git diff --cached
```
...
1. Run targeted validation through `./agent` when available.
2. Run `./agent check` for broad validation when the change is not trivially docs-only.
3. Fix any introduced errors before moving to the next file.

Do **not** commit. The user decides when to commit. For staging and commits afterward, use **`git-workflow`**.
